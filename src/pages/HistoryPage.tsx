...
const generateCsvContent = (data: LogEntry[]): string => { if (!Array.isArray(data) || data.length === 0) { return ''; } const headers = [ 'DateTime', 'SpO2', 'Pulse', 'OxygenOn', 'OxygenFlowLPM', 'Borg', 'Symptoms', 'Notes' ]; const headerRow = headers.map(escapeCsvField).join(','); const dataRows = data.map(entry => { const readableDateTime = entry.timestamp ? format(new Date(entry.timestamp), 'yyyy-MM-dd HH:mm:ss') : ''; const symptomsString = Array.isArray(entry.symptoms) ? entry.symptoms.join(' | ') : ''; const row = [ readableDateTime, entry.spo2, entry.pulse, entry.oxygenOn, entry.oxygenFlow, entry.borg, symptomsString, entry.notes ]; return row.map(escapeCsvField).join(','); }); return [headerRow, ...dataRows].join('\n'); };
